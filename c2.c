 ...
 */
static int c2_parse_grp(session_t *ps, const char *pattern, int offset,
                        c2_ptr_t *presult, int level) {
 ...
 */
static int c2_parse_target(session_t *ps, const char *pattern, int offset,
                           c2_ptr_t *presult) {
  // Initialize leaf
  ...
  unsigned tgtlen = 0;
  for (;
       pattern[offset] && (isalnum(pattern[offset]) || '_' == pattern[offset]);
       ++offset) {
    ++tgtlen;
  }
  if (!tgtlen)
...
#define TGTFILL(target, type, format)                                          \
  (pleaf->target = mstrcpy(target), pleaf->type = type, pleaf->format = format)
...
#undef TGTFILL
  ...
  if (':' == pattern[offset]) {
    ...
    enum c2_l_type type = C2_L_TUNDEFINED;
    {
      ...
      if (type) {
        if (pleaf->predef) {
          printf_errf("(): Warning: Type specified for a default target will "
                      "be ignored.");
        } else {
    ...
    if (hasformat) {
      if (pleaf->predef)
        printf_errf("(): Warning: Format \"%d\" specified on a default target "
                    "will be ignored.",
                    format);
      else if (C2_L_TSTRING == pleaf->type)
        printf_errf("(): Warning: Format \"%d\" specified on a string target "
                    "will be ignored.",
                    format);
      else {
  ...
  if (pleaf->format && 8 != pleaf->format && 16 != pleaf->format &&
      32 != pleaf->format)
...
static int c2_parse_op(const char *pattern, int offset, c2_ptr_t *presult) {
  ...
  if (C2_L_OEQ != pleaf->op && (pleaf->match || pleaf->match_ignorecase))
    c2_error(
        "Exists/greater-than/less-than operators cannot have a qualifier.");
 ...
 */
static int c2_parse_pattern(session_t *ps, const char *pattern, int offset,
                            c2_ptr_t *presult) {
  c2_l_t *const pleaf = presult->l;
    ...
    for (; pattern[offset] && delim != pattern[offset]; ++offset) {
      ...
      if ('\\' == pattern[offset] && !raw) {
        ...
        case 'x': {
          ...
          bool invalid = (pstr != &tstr[2] || val <= 0);
          free(tstr);
          if (invalid)
        ...
        default:
          c2_error("Invalid escape sequence.");
        }
      } else {
 ...
 */
static int c2_parse_legacy(session_t *ps, const char *pattern, int offset,
                           c2_ptr_t *presult) {
...
#define TGTFILL(pdefid)                                                        \
  ...
  case 'n':
    ...
    break;
  case 'i':
    TGTFILL(C2_L_PCLASSI);
    break;
  case 'g':
    TGTFILL(C2_L_PCLASSG);
    break;
  case 'r':
    TGTFILL(C2_L_PROLE);
    break;
...
static bool c2_l_postprocess(session_t *ps, c2_l_t *pleaf) {
  ...
  if (!pleaf->predef) {
    for (const char *pc = pleaf->tgt; *pc; ++pc) {
      if (islower(*pc)) {
        printf_errf("(): Warning: Lowercase character in target name \"%s\".",
                    pleaf->tgt);
        break;
      }
    }
  ...
  if (C2_L_PTSTRING == pleaf->ptntype && C2_L_MPCRE == pleaf->match) {
    ...
    pleaf->regex_pcre =
        pcre2_compile((PCRE2_SPTR)pleaf->ptnstr, PCRE2_ZERO_TERMINATED, options,
    ...
    if (jit_res < 0) {
      printf("Pattern \"%s\": PCRE2 regular expression JIT compilation failed "
             "with error code %d",
 ...
 */
static Atom c2_get_atom_type(const c2_l_t *pleaf) {
  switch (pleaf->type) {
  case C2_L_TCARDINAL:
    return XA_CARDINAL;
  case C2_L_TWINDOW:
 ...
 * For internal use.
 */
static inline void c2_match_once_leaf(session_t *ps, win *w,
                                      const c2_l_t *pleaf, bool *pres,
                                      bool *perr) {
  assert(pleaf);
  ...
  case C2_L_PTINT: {
    ...
    if (pleaf->predef) {
      *perr = false;
      ...
      case C2_L_PHEIGHTB:
        ...
        break;
      case C2_L_PBDW:
        tgt = w->a.border_width;
        break;
      case C2_L_PFULLSCREEN:
        tgt = win_is_fullscreen(ps, w);
        break;
      case C2_L_POVREDIR:
        tgt = w->a.override_redirect;
        break;
      case C2_L_PARGB:
        tgt = (WMODE_ARGB == w->mode);
        break;
      case C2_L_PFOCUSED:
      ...
      case C2_L_PWMWIN:
        ...
        break;
      case C2_L_PBSHAPED:
        tgt = w->bounding_shaped;
        break;
      case C2_L_PROUNDED:
        tgt = w->rounded_corners;
        break;
      case C2_L_PCLIENT:
        tgt = w->client_win;
        break;
    ...
    // A raw window property
      ...
      winprop_t prop = wid_get_prop_adv(ps, wid, pleaf->tgtatom, idx, 1L,
...
static bool c2_match_once(session_t *ps, win *w, const c2_ptr_t cond) {
  ...
  if (cond.isbranch) {
    ...
    case C2_B_OAND:
      result =
          (c2_match_once(ps, w, pb->opr1) && c2_match_once(ps, w, pb->opr2));
      break;
    case C2_B_OOR:
      result =
          (c2_match_once(ps, w, pb->opr1) || c2_match_once(ps, w, pb->opr2));
